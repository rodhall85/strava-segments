...
jest.mock('axios', () => ({
  get: jest.fn().mockImplementation((url) => new Promise((resolve) => {
    mockSegmentsData[0].athlete_pr_effort.elapsed_time = 31;
    mockSegmentsData[1].athlete_pr_effort.elapsed_time = 100;
    mockSegmentsData[2].athlete_pr_effort.elapsed_time = 75;
...
describe('strava api', () => {
  ...
  it('responds with the data ordered by ranking ascending', async () => {
    const expectedResponse = [1, 2, 3];

    const response = await getSegmentStats('fake_access_token');

    expect(response.map((r) => r.ranking)).toEqual(expectedResponse);
  });
});
