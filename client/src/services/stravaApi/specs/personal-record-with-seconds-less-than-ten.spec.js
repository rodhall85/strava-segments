...
jest.mock('axios', () => ({
  get: jest.fn().mockImplementation((url) => new Promise((resolve) => {
    mockSegmentsData[0].athlete_pr_effort.elapsed_time = 61;
    mockSegmentsData[1].athlete_pr_effort.elapsed_time = 71;
    mockSegmentsData[2].athlete_pr_effort.elapsed_time = 1;
...
describe('strava api', () => {
  ...
  it('should return the PR data formatted as minutes:seconds', async () => {
    ...
    const personalRecords = response.map((segment) => segment.personalRecord);

    expect(personalRecords[0]).toBe('1:01');
    expect(personalRecords[1]).toBe('1:11');
    expect(personalRecords[2]).toBe('1s');
  });
});
